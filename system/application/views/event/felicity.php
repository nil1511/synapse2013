<!-- accordion -->
<div id="accordion">

	<h2 class="current">Event Overview</h2>
	<div class="pane" style="display:block">
		<p>Are words always necessary to express yourself? Imagine a world which consists of only words which are just said and not meant! 
		Felicity is all about expressing your feelings through various forms of drama. If you can feel what you want to convey then words are not always necessary to express yourself. &ldquo;Most of the time things are conveyed without words, certain things always remain UNSPOKEN&rdquo;. Come to Felicity and let the beats of your heart be heard aloud.</p>

		<?= register_link(); ?>
	</div>
	
	<h2>Event Structure</h2>
	<div class="pane">
		<h3>Preliminary round</h3>
		<ol>
			<li>The teams have to submit a soft/hard copy of their play scripts. The script can be in English, Hindi or both.</li>
			<li>The selection of teams for stage performance will be on the basis of their script.</li>
			<li>Maximum 6 teams will be selected for phase 1 and 8 for phase 2.</li>
		</ol>

		<h3>Phase  1 (Mime drama)</h3>
		<p>Maximum time limit allotted to each team will be 10-15 minutes.</p>



		<h3>Phase 2 (Musical drama)</h3>
		<ol>
			<li>Instead of using dialogues music should be used as a tool to convey the message.  Music should be prepared from the list of songs provided by us. Self composed parodies are also allowed.</li>
			<li>Maximum time limit allotted to each team will be 15-20 minutes.</li>
		</ol>
	</div>
	
	<h2>Rules</h2>
	<div class="pane">
		<h3>General rules</h3>
		<ul>
			<li>Multiple entries from a particular college/institution are allowed.</li>
			<li>The decision of the judges and the co-ordinators shall be final and binding to all teams.   No cribbing shall be entertained in this regard.</li>
			<li>Maximum team size is 12 including backstage/technical crew.</li>
			<li>All team members must be students of the college/institute carrying valid identity cards. </li>
			<li>Clothing or actions that could be perceived as obscene, lewd, vulgar or plainly offensive is strictly prohibited.</li>
			<li>No costumes will be provided by us.</li>
			<li>Negative marks for exceeding the time limit, but exceeding it by 5 minutes would disqualify the team. </li>
			<li>In case of any situations which have not been described in the rules, the decision taken by the judges will be final.</li>
		</ul>
		
		<h3>Rules for phase 1 and 2</h3>
		<ul>
			<li>Submit soft copy of the prepared music piece at least two hours before the beginning of the event to event co-ordinators.</li>
			<li>No musical instruments and props will be provided by us. However any props and musical instruments can be used. (Innovative musical instruments and self composed music and lyrics may fetch you more points). </li>
		</ul>
	</div>
	
	<h2>Co-ordinators</h2>
	<div class="pane">
    <ul>
<li>Prateek Jain - 7567249439
<li>Sahil Shah - 9033666898
</ul>

	</div>
</div>