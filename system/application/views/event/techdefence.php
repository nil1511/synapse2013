<!-- accordion -->
<div id="accordion">

    <h2 class="current">Event Overview</h2>
    <div class="pane" style="display:block">
		<br />
		<p>In this era, every person is networked with the planet. Many modes of communication have come to exist, of which the most used is Internet. As Internet users increase each and every day, Security has become a major Issue. New technologies create new criminal opportunities and new types of crime.  It is not impossible for hackers to extract our information in a fraction of minute. Today Cyber Crime has extended across a spectrum of activities ranging from e-mail to SMS and call spoofing, Credit Card Fraud, phishing and Remote key logging etc. As Cyber Crime is increasing every day with advancing technologies, it has become our responsibility to protect ourselves from becoming a victim. Synapse 2013 has taken a step to reduce Cyber Crime by presenting Hack track 2.0 workshop in association with TechDefence. The workshop will be of 2 days scheduled on 16th and 17th Feb.Last date of Registration for DA-IICT Student is 14th Feb</p>
		REGISTRATION CLOSED
		<!--<label>For DA-IICT Students</label>	<?= register_link(); ?>
		<label>For Non DA-IICT Students  those who want to register online <a href="http://event.ayojak.com/event/e-cell-iit-delhi-techdefence-presents-hacktrack-v2-0-at-daiict-gandhinagar/ticket-widget" target="_blank">Register for Tech Defence(Non DA-IICT)</a></label>
		<p>For Those who want to pay on the spot register <a href="http://synapse.daiict.ac.in/2013/index.php/register/techdefence" target="_blank">here</a>-->
    </div>
    
    <h2>Details Of Workshop</h2>
	<div class="pane">
	<ul>
The workshop contents are designed by TechDefence team for providing intensive knowledge on hacking and Information Security. <br/>
Day 1:
<li>Cyber Ethics
<li>Web Application and Security
<li>Information Gathering and Security
<li>Wireless Security
<li>Android Hacking and Security
<li>Mobile, Voip Hacking and Security
<li>E-mail Hacking and Security
<li>System and Network Hacking
<li>Google Hacking 
<li>Trojans, Backdoors
<li>Cyber Forensics
<li>Investigation Methodologies
<li>Introduction to Cyber Crime
<br />
<li>For detailed description about day 1 content, visit the following link :
<a href="https://www.dropbox.com/s/gdmamx5gw4q43wp/Level%201.pdf" target="_blank">Level 1</a></ul>
<br />
Day 2:
<ul>
<li>Info Gathering using Linux
<li>Windows Memory Management
<li>Buffer Overflows
<li>Assembly Language Basics
<li>Metasploit Framework
<li>Reverse Engineering
<li>Wireless Hacking
<br />
<li>For detailed description about day 1 content, visit the following link :
<a href="https://www.dropbox.com/s/3zdodcvk6hphz04/Level%202.pdf" target="_blank">Level 2</a><ul>
    </div>
    <h2>Benefits of Workshop</h2>
    <div class="pane">
	<br />
<ul>
<li>Hands on Demonstration of Latest Hacking Techniques & tools
<li>Hands on Practice Sessions
<li>Ethical Hacking Toolkit (Includes PowerPoint Presentations, Tools, EBooks) worth Rs 600

</ul>

<ul>
<li>Certificates of certified Ethical hacking Expert Level-1 to students from E-CELL IIT Delhi, TechDefence and Synapse, DA-IICT
<li>Special Discount offer to the college students to join 1 month training program conducted by TechDefence at their centers for Ethical Hacking and Cyber Security
<li>TechDefence will be  providing a free internship to the topper of each workshop at any of the centers of TechDefence for an entire Certified  Cyber Security Expert Course of worth Rs.15000
<li>TechDefence will also provide the study kits including a (10 DVD set + 2 books) exclusively designed on Ethical Hacking and Information Security to the topper of each workshop of the series of all workshops.
<li>Unique Barcode to Access to Hack Track Portal for 2 years
</ul>
<br />

HackTrack Portal Features:
<ul>    
<li>Support for 2 years
<li>Access to Online Competitions.
<li>Access to Monthly Webinars
<li>Internship/Career Opportunities from TechDefence and its associates
<li>Monthly Newsletters & Knowledgebase Section
<li>Resume Submission
</ul>
    </div>
	<h2>WorkShop Highlights</h2>
    <div class="pane">
	<ul>
<li>Learn and interact with Renowned Industry Experts
<li>Receive an unparalleled education on the art of computer security 
<li>Hands on Demonstration of Latest Hacking Techniques & tools
<li>Hands on Practice Sessions
<li>Hands on demonstration of various cases solved by Sunny Vaghela 
<li>PowerPoint presentation, Live Demos, Interactive Question and Answer sessions and comprehensive reading material
<li>Toppers of each workshop to be personally interviewed with sunny Vaghela and will receive a chance to work with him on security projects.
</div>

    <h2>About TechDefence</h2>
    <div class="pane">
<a href="http://techdefence.com" target="_blank"><img src="<?=  base_url();?>images/techdefence.png" style="padding-left:35%;"/></a>
	<ul>
	<li>TechDefence has been awarded as No.1 Ethical Hacking Training & Coaching Company of INDIA by NBC Awards. TechDefence has trained professionals from reputed corporates like Yahoo, Google, ISACA, k7 Antivirus, ZOHO, TCS, HCL, Elitecore Technologies & Indian Oil. We have solved several cases like Fake Profile Cases, Email Spoofing cases, Phishing case, Espionage cases, Credit Card fraud Cases, Cyber Pornography Case, SMS Spoofing Case in association with Crime Branch, Ahmedabad. The workshops are specifically designed to provide the right knowledge to the aspiring Hacking and Information Security aspirants and has benefited over 25000 students and professionals from over 300 top Institutes and Universities across India and abroad.
<li>To know more about TechDefence and the awards won by it, visit the following link :

<li><a href="https://www.dropbox.com/s/ldyqntkm1dwchan/TechDefence%20%28Company%20Profile%29.pdf" target="_blank">About TechDefence</a> 

<li><a href="https://www.dropbox.com/s/34w6hfeqgqwbsvo/TechDefence%20%28Awards%20%26%20Media%20accolades%29.pdf" target="_blank">Awards and Media accolades</a>

    </div>
	
	<h2>Registration Details</h2>
    <div class="pane">
		<p>
<b>General Guidelines :</b>
<ul>
<li>Dates of workshop are 16th and 17th February-2013.
<li>Timing will be approx. 8 hours per day.
<li>Venue of Workshop is Lecture Theater 1, DA-IICT.
<li>Workshop is open for everyone irrespective of their profession.
<li>We have only limited seats for the workshop.
<li>Certain amounts of seats are reserved for non-DA-IICT participants.
<li>All the Participants have to carry their ID-card during the workshop.
<li>You will also be provided with ID card for the workshop.
<li>Participants can bring their laptops.
<li>Internet facility will be provided during the workshop</ul>
<br />
Registration guidelines :
<ul>
<li>Registration fee will be Rs 1100/- inclusive of all taxes.
<li>Registration money will cover :
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Fee for two day workshop
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Software toolkit
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Certificate by E-Cell IIT Delhi, TechDefence and Synapse 	
<li>Deadline for registration is 14th of February or till the seats last.
<li>Registration will be completed only after we receive the registration fee. 
<li>Registration money is not refundable under any circumstances.
<li>On spot registrations will be conducted.
</ul>
<br />        
Payment Guidelines :
<ul>
<li>All the DA-IICT participants have to submit the registration money to the organizers in person. 
<li>All the non-DA-IICT participants should choose either of the following methods:
</ul><br/>
1)Give money in person to the volunteers who will come to your college for publicity. No need to register on the website in this case.
<br>
<br />
2)Online registration and payment can be done <a href="http://event.ayojak.com/event/e-cell-iit-delhi-techdefence-presents-hacktrack-v2-0-at-daiict-gandhinagar/ticket-widget" target="_blank">here</a>
<br>
<br/>
3)You Can Pay Money on the spot during the workshop
<br><br>4)Submit the money in person to the organizers.<br/>
You don’t have to register on the website if you are choosing this method.<br/>
<ul>
<li>Any other method of payment other than these will not be accepted.
<li>You can make payments for more than one participant together. But each of them has to register on the website separately.</ul>
</ul>

</p>
    </div>
    <h2>Contacts</h2>
    <div class="pane"><br/>
   For Information regarding Workshop you can contact the following persons :<br/>
   <br/><br/>
Nimit Agarwal <br/>
Ph. no: 8905811446<br/>
Room no- D307<br/>
E-mail: nimit.agrwal@gmail.com<br/>
<br/>
Petlu Krishna Chaitanya<br/>
		Ph. no: 8000335829<br/>
		Room no. – D317<br/>
		E-mail: pkchaitanya9737836133@gmail.com<br/>
	</ul>
    </div>
    
    
</div>